...
class SubagentMarkerMiddleware(AgentMiddleware):
    """Middleware that marks AI messages as coming from a subagent.
    
    Adds `is_subagent=True` to the `additional_kwargs` of all AI messages
    generated by the subagent, allowing the frontend to identify and filter
    subagent messages.
    """
    ...
    def __init__(self, subagent_name: str = ""):
        ...
        Args:
            subagent_name: The name of the subagent for logging/debugging.
        """
    ...
    def wrap_model_call(
        ...
        """Intercept model call to add subagent marker to response."""
        ...
        if response.result and isinstance(response.result[0], AIMessage):
            ...
            # Create new response with updated message
            # ModelResponse is a dataclass, result is a list of messages
            new_result = [new_message] + response.result[1:]
    ...
    async def awrap_model_call(
        ...
        """Intercept async model call to add subagent marker to response."""
    ...
    *,
    ...
    if general_purpose_agent:
        general_purpose_middleware = [
            SubagentMarkerMiddleware(subagent_name="general-purpose"),  # Mark messages as from subagent
    ...
    for agent_ in subagents:
        ...
        # Add SubagentMarkerMiddleware at the start to mark all messages
        _middleware = [
            SubagentMarkerMiddleware(subagent_name=agent_["name"]),
            *default_subagent_middleware,
            *(agent_["middleware"] if "middleware" in agent_ else []),
        ]
...
def _create_task_tool(
    *,
    ...
    def task(
        ...
        # Inject subagent markers into config
        config = runtime.config.copy() if runtime.config else {}
